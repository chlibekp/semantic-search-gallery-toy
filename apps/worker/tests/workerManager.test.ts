...
describe("WorkerManager", () => {
  ...
  it("Should throw error when start is called multiple times", async () => {
    ...
    await expect(workerManager.start()).rejects.toThrow(
      "WorkerManager is already running",
    );
  ...
  it("Should handle failed jobs and retry", async () => {
    ...
    const processingQueue = await redis.lrange(
    ...
    expect(
      failedQueue.length + processingQueue.length + mainQueue.length,
    ).toBeGreaterThan(0);
  }, 10000);
});
